...
?>
	<ul class="wc-booking-summary-list">
		...
		<?php if ( $resource ) : ?>
			...
			if ( get_post_meta( $booking->get_id(), 'auto_assigned') ) {
				
				echo "Bed: I accept the bed available at the the check in";

			} else {
			
				foreach ( $resource as $res ) {
			    		$recurso = $product->get_resource( $res );
			    		echo esc_html( sprintf( __( '%1$s: %2$s', 'woocommerce-bookings' ), "Bed", $recurso->get_name() ) );
				}
			}
			?>
			</li>
		...
		<li><?php echo '<span>'.__('Cruise Line').': ' . get_post_meta( $booking->get_order()->id, 'cruise_line', true ) . '</span>'; ?></li>
        <li><?php echo '<span>'.__('Arrival Time').': ' . get_post_meta( $booking->get_order()->id, 'arrival_time', true ) . '</span>';  ?></li>
	</ul>
